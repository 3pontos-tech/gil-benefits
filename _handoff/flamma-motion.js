  ...
  function tagCompanies() {
    ...
    if (flow) {
      ...
      $$('article', flow).forEach(function (card, i) {
        ...
        addClass($('img[class*="object-contain"]', card), 'fm-card-prod');
    ...
    if (tool) {
      var tGrid = $('.grid', tool);
      if (tGrid && tGrid.children[0]) {
        Array.prototype.slice.call(tGrid.children[0].children).forEach(function (box, i) {
    ...
    if (privacy) {
      var pGrid = $('.grid', privacy);
      if (pGrid) {
        reveal(pGrid.children[0], 'reveal-left');
        reveal(pGrid.children[1], 'reveal-right');
      }
    }
    ...
    if (cta) {
      ...
      byClass(cta, 'aspect-[762/557]').forEach(function (f) { addClass(f, 'fm-zoom'); });
    }
