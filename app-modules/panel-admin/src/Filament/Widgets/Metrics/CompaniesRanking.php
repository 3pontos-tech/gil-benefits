...
class CompaniesRanking extends TableWidget
{
    ...
    public function table(Table $table): Table
    {
        ...
        return $table
            ->heading(__('panel-admin::widgets.metrics.companies_ranking.heading'))
            ->searchable(false)
            ->query(
                Company::query()
                    ->withCount([
            ...
            ->columns([
                TextColumn::make('name')
                    ->label(__('panel-admin::widgets.metrics.companies_ranking.column_company'))
                    ->searchable(),
                TextColumn::make('total_appointments')
                    ->label(__('panel-admin::widgets.metrics.companies_ranking.column_total'))
                    ->sortable(),
                TextColumn::make('completed_appointments')
                    ->label(__('panel-admin::widgets.metrics.companies_ranking.column_completed'))
                    ->sortable(),
                TextColumn::make('pending_appointments')
                    ->label(__('panel-admin::widgets.metrics.companies_ranking.column_pending'))
                    ->sortable(),
                TextColumn::make('completion_rate')
                    ->label(__('panel-admin::widgets.metrics.companies_ranking.column_completion_rate'))
                    ->state(fn (Company $record): string => $record->total_appointments > 0
