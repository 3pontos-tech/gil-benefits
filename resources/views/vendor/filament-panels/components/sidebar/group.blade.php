{{--
    Cópia de vendor/filament/filament/resources/views/components/sidebar/group.blade.php.

    Único motivo do override: o layout do painel do colaborador pede uma frase
    descritiva sob o título de cada grupo, e o NavigationGroup do Filament não tem
    campo de descrição (nem esta view recebe o objeto do grupo, só o rótulo).

    A única alteração em relação ao original é o bloco marcado com
    "ALTERAÇÃO LOCAL" abaixo. Ao atualizar o Filament, re-sincronizar o resto.
--}}
@props([
    'active' => false,
    'collapsible' => true,
...
@php
    ...
    // ALTERAÇÃO LOCAL
    $groupDescription = \TresPontosTech\PanelApp\Support\NavigationGroupDescriptions::for($label);
@endphp
...
>
    ...
    {{-- ALTERAÇÃO LOCAL: frase descritiva do grupo, escondida junto com o rótulo
    ...
    @if (filled($groupDescription))
        <p
            @if ($sidebarCollapsible)
                ...
                x-transition:enter-end="fi-transition-enter-end"
            @endif
            class="fi-sidebar-group-description"
        >
            {{ $groupDescription }}
        </p>
    @endif
